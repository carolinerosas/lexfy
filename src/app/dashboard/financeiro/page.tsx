...
export default function FinanceiroPage() {
  ...
  const load = useCallback(() => {
    setHonorarios(
      getHonorariosWithProcesso().sort(
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.map((h) => (
                <tr key={h.id} className="hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    <button onClick={() => { deleteHonorario(h.id); load(); }} className="text-gray-300 hover:text-red-500 transition-colors">
...
function SummaryCard({ icon, bg, label, value, textColor }: { icon: React.ReactNode; bg: string; label: string; value: string; textColor?: string }) {
